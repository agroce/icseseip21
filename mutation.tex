...
To put our work on Bitcoin in context, we perform mutation analysis for other popular cryptocurrencies.
...
We executed mutant files against each project's default test suite (determined by consulting READMEs and build documentation) using universal mutator to obtain a final mutation score. The runtime
...
with examples of surviving mutants. One commonly used operator comments out a
line of source code; live mutants thus typically corresponding to uncovered lines.
...
The Bitcoin Core mutation score (we use the score before manual
pruning of equivalent mutants, since we did not perform pruning
...
quality and testing, which is likely to related (whether directly or
...
alternative cryptocurrencie (e.g., Ethereum) make use of fuzz testing (i.e., are
...
quality as measured by coverage and mutation score. Other projects should
certainly ensure that the totality of their test efforts (which typically go
beyond what we evaluate here), are of sufficient quality.  Even with the
limitations of our evaluation, there is clearly room for improvement in the
functional test coverage (and mutation score) for a number of popular
cryptocurrencies.  
\clg{Alex or Rijnard can haggle out what this last sentence should say, if you
  don't like what my sentence said; I'm trying to thread the needle.}
...
cryptocurrencies, at least for the purposes of central logic (like transaction
validation). It also \emph{should} in principle be reasonably feasible, in
principle; we found it tractable, once we isolated key functionality and a test
suite. However, our experience suggests that the present state of these projects
and their practices renders it difficult to conduct such evaluations
comprehensively. It can be difficult to identify and isolate critical code for
the purposes of evaluating the quality of the associated test suite. It was
difficult even to obtain code coverage data for those projects that do not
report it via continuous integration.

\clg{Pithy summary/call to arms? I'd love for Rijnard to give it a shot,
  honestly, I think he has a better vision here.}
